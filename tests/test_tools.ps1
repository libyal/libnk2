...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$ResultRun = RunTestBinary ${TestExecutablesDirectory} "nk2_test_tools_${TestName}"
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$ResultRun = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "nk2info" ${TestProfile} "" ${TestInput}
