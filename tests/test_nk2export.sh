...
TEST_EXECUTABLE="../nk2tools/nk2export";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../nk2tools/nk2export.exe";
fi
		...
		do
			...
			do
				...
				then
					...
					run_test_on_input_file "${TEST_SET_DIRECTORY}" "nk2export" "with_callback" "${OPTION_SET}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}" "${OPTIONS[@]}";
					RESULT=$?;
			...
			then
				run_test_on_input_file "${TEST_SET_DIRECTORY}" "nk2export" "with_callback" "" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}";
				RESULT=$?;
			fi
