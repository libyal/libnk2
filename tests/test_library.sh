...
LIBRARY_TESTS="error file_footer file_header io_handle item notify record_entry";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./nk2_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libnk2" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
