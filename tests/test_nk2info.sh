...
TEST_EXECUTABLE="../nk2tools/nk2info";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../nk2tools/nk2info.exe";
fi
		...
		do
			...
			do
				...
				then
					...
					run_test_on_input_file "${TEST_SET_DIRECTORY}" "nk2info" "with_stdout_reference" "${OPTION_SET}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}" "${OPTIONS[@]}";
					RESULT=$?;
			...
			then
				run_test_on_input_file "${TEST_SET_DIRECTORY}" "nk2info" "with_stdout_reference" "" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}";
				RESULT=$?;
			fi
