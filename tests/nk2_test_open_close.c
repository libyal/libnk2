 ...
 */
int nk2_test_single_open_close_file(
     ...
     int expected_result )
{
	...
	if( access_flags == LIBNK2_OPEN_READ )
	{
		access_string = "read";
	}
	else if( access_flags == LIBNK2_OPEN_WRITE )
	{
		access_string = "write";
	}
	else
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int nk2_test_multi_open_close_file(
     system_character_t *filename,
     int access_flags,
     int expected_result )
{
...
#endif
{
...
#if defined( HAVE_DEBUG_OUTPUT ) && defined( NK2_TEST_OPEN_CLOSE_VERBOSE )
	 ...
	 */
	if( nk2_test_single_open_close_file(
	     source,
	     LIBNK2_OPEN_READ,
	     1 ) != 1 )
	{
	 ...
	 */
	if( nk2_test_multi_open_close_file(
	     source,
	     LIBNK2_OPEN_READ,
	     1 ) != 1 )
	{
