/*
 * Signal handling functions for the nk2tools
 *
...
void (*nk2signal_signal_handler)( nk2signal_t ) = NULL;
...
{
	static char *function = "nk2signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( nk2signal_signal_handler != NULL )
			{
				nk2signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "nk2signal_attach";
	...
	nk2signal_signal_handler = signal_handler;
	...
	nk2signal_initialize_memory_debug();
...
int nk2signal_detach(
     void )
{
	static char *function = "nk2signal_detach";
