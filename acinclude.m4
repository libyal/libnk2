...
AC_DEFUN([LIBNK2_CHECK_FUNC_LANGINFO_CODESET],
 ...
 AS_IF(
  ...
  [AC_CACHE_CHECK(
   ...
   [ac_cv_libnk2_langinfo_codeset],
   [AC_LANG_PUSH(C)
   AC_LINK_IFELSE(
    [AC_LANG_PROGRAM(
     [[#include <langinfo.h>]],
     [[char* charset = nl_langinfo( CODESET );]]) ],
    [ac_cv_libnk2_langinfo_codeset=yes],
    [ac_cv_libnk2_langinfo_codeset=no])
   AC_LANG_POP(C) ]) ],
  [ac_cv_libnk2_langinfo_codeset=no])

 AS_IF(
  [test "x$ac_cv_libnk2_langinfo_codeset" = xyes],
  [AC_DEFINE(
...
AC_DEFUN([LIBNK2_CHECK_FUNC_PRINTF_JD],
 [AC_MSG_CHECKING(
 ...
 AC_LINK_IFELSE(
  [AC_LANG_PROGRAM(
   [[#include <stdio.h>]],
   [[printf( "%jd" ); ]] )],
  [ac_cv_libnk2_have_printf_jd=no],
  [ac_cv_libnk2_have_printf_jd=yes])

 dnl Second try to see if compilation and linkage with a parameter succeeds
 AS_IF(
  [test "x$ac_cv_libnk2_have_printf_jd" = xyes],
  [AC_LINK_IFELSE(
   [AC_LANG_PROGRAM(
    [[#include <sys/types.h>
#include <stdio.h>]],
...
if( ( string[ 0 ] != '1' ) || ( string[ 1 ] != '0' ) ) return( 1 ); ]] )],
    [ac_cv_libnk2_have_printf_jd=yes],
    [ac_cv_libnk2_have_printf_jd=no],
    [ac_cv_libnk2_have_printf_jd=undetermined])
   ])
...
AC_DEFUN([LIBNK2_CHECK_FUNC_PRINTF_ZD],
 ...
 AC_LINK_IFELSE(
  [AC_LANG_PROGRAM(
   [[#include <stdio.h>]],
   [[printf( "%zd" ); ]] )],
  [ac_cv_libnk2_have_printf_zd=no],
  [ac_cv_libnk2_have_printf_zd=yes])

 dnl Second try to see if compilation and linkage with a parameter succeeds
 AS_IF(
  [test "x$ac_cv_libnk2_have_printf_zd" = xyes],
  [AC_LINK_IFELSE(
   [AC_LANG_PROGRAM(
    [[#include <sys/types.h>
#include <stdio.h>]],
...
if( ( string[ 0 ] != '1' ) || ( string[ 1 ] != '0' ) ) return( 1 ); ]] )],
    [ac_cv_libnk2_have_printf_zd=yes],
    [ac_cv_libnk2_have_printf_zd=no],
    [ac_cv_libnk2_have_printf_zd=undetermined])
   ])
 ...
 AS_IF(
  ...
  AC_LINK_IFELSE(
   AC_LANG_PROGRAM(
    ...
    ac_cv_libnk2_ctime_r_size=yes],
    [ac_cv_libnk2_ctime_r_size=no])

  AS_IF(
   [test "x$ac_cv_libnk2_ctime_r_size" = xno],
   [AC_LINK_IFELSE(
    AC_LANG_PROGRAM(
     [[#include <time.h>]],
     [[ctime_r( NULL, NULL )]]),
    [AC_MSG_RESULT(
     [with two arguments])
    ac_cv_libnk2_ctime_r_posix=yes],
    [ac_cv_libnk2_ctime_r_posix=no])
   ])

  AS_IF(
   [test "x$ac_cv_libnk2_ctime_r_posix" = xno],
  ...
  AS_IF(
   [test "x$ac_cv_libnk2_ctime_r_size" = xno && test "x$ac_cv_libnk2_ctime_r_posix" = xno],
   [AC_MSG_WARN(
    [unknown])
  ...
  AS_IF(
   [test "x$ac_cv_libnk2_ctime_r_size" = xyes],
   [AC_DEFINE(
    [HAVE_CTIME_R_SIZE],
    [1],
...
AC_DEFUN([LIBNK2_CHECK_FUNC_MKDIR],
...
#include <sys/types.h>]],
    ...
    ac_cv_libnk2_mkdir_mode=yes],
    [ac_cv_libnk2_mkdir_mode=no])

  AS_IF(
   [test "x$ac_cv_libnk2_mkdir_mode" = xno],
   [AC_LINK_IFELSE(
    AC_LANG_PROGRAM(
     [[#include <io.h>]],
     [[mkdir( "" )]]),
    ...
    ac_cv_libnk2_mkdir=yes],
    [ac_cv_libnk2_mkdir=no])
   ])
  ...
  AS_IF(
   [test "x$ac_cv_libnk2_mkdir_mode" = xno && test "x$ac_cv_libnk2_mkdir" = xno],
   [AC_MSG_WARN(
    [unknown])
   ac_cv_func_mkdir=no])
  ...
  AS_IF(
   [test "x$ac_cv_libnk2_mkdir_mode" = xyes],
   [AC_DEFINE(
    [HAVE_MKDIR_MODE],
    [1],
